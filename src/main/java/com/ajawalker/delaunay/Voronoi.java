...
public class Voronoi {
	...
	public static void main(String[] args) {
		...
		Vertex[] points = new Vertex[10000];
		for (int i = 0; i < points.length; i++) {
			points[i] = new Vertex(rnd.nextDouble() * 400, rnd.nextDouble() * 400);
		}
		Voronoi vor = new Voronoi(0.0);

		long startNanos = System.nanoTime();
		List<GraphEdge> edges = vor.generateVoronoi(points);
		System.out.println((System.nanoTime() - startNanos) / 1000000000.0);

		BufferedImage image = new BufferedImage(400, 400, BufferedImage.TYPE_INT_RGB);
		Graphics g = image.getGraphics();
		g.setColor(Color.white);
		for (GraphEdge edge : edges) {
			g.drawLine((int)edge.xPos1, (int)edge.yPos1, (int)edge.xPos2, (int)edge.yPos2);
		}
		FrameViewer viewer = new FrameViewer("Voronoi", 400, 400);
